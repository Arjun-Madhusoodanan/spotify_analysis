...
# df.to_csv('myTracksforAnalysis.csv', index=False)


# Function to fetch recently played tracks
def get_recently_played(token, after_timestamp):
...
# Function to gather listening history for the past year
def get_listening_history(token, start_time):
    all_tracks = []
    after_timestamp = int(start_time.timestamp() * 1000)
    
    while True:
        recently_played = get_recently_played(token, after_timestamp)
...
print(one_year_ago)
listening_history = get_listening_history(token, one_year_ago)

# Create a DataFrame with the listening history
track_data = []
